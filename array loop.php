...
if (isset($_POST['first_name'], $_POST['last_name'], $_POST['email'], $_POST['phone'], $_POST['dob'], $_POST['gender'], $_POST['languages'],$_POST['password'])) {
    ...
    if (!empty($first_name) && !empty($last_name) && !empty($email) && !empty($phone) && !empty($dob)&& !empty($password) && !empty($gender) && !empty($languages)) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $insert = "INSERT INTO user_register (first_name, last_name, email,phone,dob,password,gender,languages,role) VALUES ('$first_name', '$last_name', '$email', '$phone', '$dob','$password','$gender','$languages','$role')";
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response === "user") {
                ...
                $('#general_error_message').text('Update failed.');
            }
        }
    });
...
if ($_SESSION['role'] != 'admin') {
    ...
    $check = mysqli_query($conn, "SELECT id FROM user_register WHERE email='$session_email'");
...
<?php
$sql = "SELECT * FROM user_register WHERE role='user'";
$result = mysqli_query($conn, $sql);
...
<tr id="row<?php echo $row['id']; ?>">
    ...
    <td><button class="edit" data-id="<?php echo $row['id']; ?>">Edit</button></td>
    <td><button class="delete" data-id="<?php echo $row['id']; ?>">Delete</button></td>
</tr>
<?php } ?>
</table>
...
$(document).ready(function(){
    ...
    $('.edit').click(function(){
        var id = $(this).data('id');
        $.post("user-update.php",{id:id},function(response){
            $('body').html(response);
        });
    });
    ...
    $('.delete').click(function(){
        if(confirm("Are you sure to delete this user?")){
            var id = $(this).data('id');
            $.post("delete-user.php",{id:id},function(){
                $('#row'+id).remove();
            });
        }
    });
